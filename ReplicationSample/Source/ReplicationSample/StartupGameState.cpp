...
AStartupGameState::AStartupGameState(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{	
	OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AStartupGameState::OnCreateSessionComplete);
	OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AStartupGameState::OnStartOnlineGameComplete);
	OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AStartupGameState::OnFindSessionsComplete);
	OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AStartupGameState::OnJoinSessionComplete);
	OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AStartupGameState::OnDestroySessionComplete);
}
...
void AStartupGameState::StartOnlineGame()
{
...
void AStartupGameState::FindOnlineGames()
{
	const auto Instance = GetGameInstance();
	const ULocalPlayer* const Player = Instance->GetFirstGamePlayer();
...
void AStartupGameState::JoinOnlineGame()
{
	const auto Instance = GetGameInstance();
	const ULocalPlayer* const Player = Instance->GetFirstGamePlayer();
...
void AStartupGameState::DestroySessionAndLeaveGame()
{
...
bool AStartupGameState::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
...
void AStartupGameState::FindSessions(TSharedPtr<const FUniqueNetId> UserId, bool bIsLAN, bool bIsPresence)
{
	if (const IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get())
	{
		if (const IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface(); Sessions.IsValid() && UserId.IsValid())
...
bool AStartupGameState::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
...
void AStartupGameState::DestroySession() const
{
	if (const IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get())
	{
		if (const IOnlineSessionPtr Sessions = OnlineSub->GetSessionInterface(); Sessions.IsValid())
...
void AStartupGameState::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
...
void AStartupGameState::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
...
void AStartupGameState::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AStartupGameState::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
void AStartupGameState::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
